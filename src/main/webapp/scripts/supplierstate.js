        ...
        if (result.resCode == "0000") {
          ...
          for(var i=0;i<result.data.companies.length;i++){
               html=html+"<tr><td>"+result.data.companies[i].comname+"</td><td><select><option>"
                                   +result.data.items[0].pstatus+"</option><option>进行商务洽谈</option><option>达成初步意向</option><option>进行合同洽谈</option><option>合同达成意向</option><option>进行合同审核</option><option>合同审核完成</option><option>进行合同用印</option><option>合同签署完成</option><option>尚未预付款</option><option>已经预付款</option><option>月结</option><option>其它</option></select></td><td>"
                                   +result.data.items[0].uname+"</td><td>"
                                   +result.data.items[0].time+"</td><td><select><option>"
                                   +result.data.items[1].pstatus+"</option><option>账号尚未开通</option><option>申请信息准备</option><option>申请测试账号</option><option>进行测试对接</option><option>测试对接完成</option><option>申请生产账号</option><option>进行生产对接</option><option>生产对接完成</option><option>其它</option></select></td><td>"
