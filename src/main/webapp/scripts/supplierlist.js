...
$.ajax({
    url: "http://localhost:8888/company/companySelective?searchWord=&type=&typeId=1",
    type: "GET",
    ...
    success: function(result) {
        if (result.resCode == "0000") {
            ...
            for (var i = 0; i < result.data.companies.length; i++) {
                html = html + "<tr><td>" + result.data.companies[i].comname + "</td><td>"
                + result.data.companies[i].comcontact + "</td><td>"
                + result.data.companies[i].comemail + "</td><td>"
                + result.data.companies[i].comaddr + "</td><td><a id='"
                + result.data.companies[i].comid+"' target='_blank' class='btn btn-info btn-sm' href='/views/supplierdetail.html?comid="+result.data.companies[i].comid+"'>详情</a></td><td>"
                + result.data.companies[i].createtime +
...
$("#search_id").click(function(){
    ...
    $.ajax({
      url: "http://localhost:8888/company/companySelective?&type=&typeId=0",
      type: "GET",
      contentType: "application/json;charset=utf-8",
      dataType: "json",
...
$("#create_ok").click(function(){
    ...
    $.ajax({
      url: "http://localhost:8888/company/modifyCompany",
      type: "POST",
      contentType: "application/json;charset=utf-8",
      dataType: "json",
      data:JSON.stringify(postData),
...
$('#supplierlist').on('click', function(e) {
    ...
    if (type == 'button' && operate == '删除') {
      $('#delete').on('click', function(){
        ...
        let company = data.companies[name];
        let item = new Array;
        item.push(data.items[name*3]);
        item.push(data.items[name*3 - (-1)]);
        item.push(data.items[name*3 - (-2)]);
